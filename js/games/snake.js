...
function createDpadOverlay() {
    ...
    dpadOverlay.style.cssText = `
        ...
        opacity: 0.7;
    `;
    dpadOverlay.innerHTML = `
        <div style="position: absolute; top: 10px; left: 50%; transform: translateX(-50%); width: 50px; height: 50px; background: rgba(255,255,255,0.6); border-radius: 50%; user-select: none; cursor: pointer; border: 2px solid rgba(255,255,255,0.8);" data-dir="up"></div>
        <div style="position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); width: 50px; height: 50px; background: rgba(255,255,255,0.6); border-radius: 50%; user-select: none; cursor: pointer; border: 2px solid rgba(255,255,255,0.8);" data-dir="down"></div>
        <div style="position: absolute; left: 10px; top: 50%; transform: translateY(-50%); width: 50px; height: 50px; background: rgba(255,255,255,0.6); border-radius: 50%; user-select: none; cursor: pointer; border: 2px solid rgba(255,255,255,0.8);" data-dir="left"></div>
        <div style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); width: 50px; height: 50px; background: rgba(255,255,255,0.6); border-radius: 50%; user-select: none; cursor: pointer; border: 2px solid rgba(255,255,255,0.8);" data-dir="right"></div>
    `;
    document.body.appendChild(dpadOverlay);

    // Add touch handlers to d-pad buttons
    dpadOverlay.addEventListener('touchstart', (e) => {
        e.preventDefault();
        const target = e.target.closest('[data-dir]');
        if (target && target.dataset.dir) {
            snakeDir = target.dataset.dir;
            // Simple feedback: brighter and slightly larger
    ...
    dpadOverlay.addEventListener('touchend', (e) => {
        const target = e.target.closest('[data-dir]');
        if (target) {
            // Reset to subtle and normal size
            target.style.background = 'rgba(255,255,255,0.6)';
            target.style.transform = target.style.transform.replace(' scale(1.2)', '');
        }
    });
}
