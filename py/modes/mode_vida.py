...
class ModeVida(BaseMode):
    def __init__(self, midi_out, config=None):
        ...
        self.arp_position = 0
        self.velocity = 70
        self.octave_offset = 0  # Offset d'octava controlat per pot Y
        self.selected_progression = 0  # Índex de progressió seleccionada per pot Z
        
        # Definir progressions d'acords harmoniosos BASE (sense offset)
        self.base_progressions = [
        ...
        # Llista de progressions actuals (amb offset aplicat)
        self.chord_progressions = [prog[:] for prog in self.base_progressions]
    ...
    def _select_new_chord_progression(self, index=None):
        """Selecciona una nova progressió d'acords"""
        if index is not None:
            progression = self.chord_progressions[index % len(self.chord_progressions)]
        else:
    ...
    def update(self, pot_values, button_states):
        ...
        # Duració dels acords (sempre present)
        self.chord_duration = 4.0 + (x / 127.0) * 12.0  # 4-16 segons
        
        # POT X: Velocitat (INVERTIT) - Més valor = MÉS RÀPID (menys interval)
        self.note_interval = 1.0 - (x / 127.0) * 0.8  # 1.0-0.2 segons (invertit)
        
        # POT Y: Octava (0-3 octaves de diferència)
        new_octave = int((y / 127.0) * 3)  # 0, 1, 2 o 3 octaves
        if new_octave != self.octave_offset:
            self.octave_offset = new_octave
            self._apply_octave_offset()
        
        # POT Z: Tipus de progressió (0, 1 o 2)
        progression_index = int((z / 127.0) * (len(self.base_progressions) - 0.01))
        if progression_index != self.selected_progression:
            self.selected_progression = progression_index
            self._apply_octave_offset()  # Aplicar octava i canviar progressió
    ...
    def _apply_octave_offset(self):
        """Aplica l'offset d'octava i la progressió seleccionada als acords"""
        # Aplicar offset d'octava
        octave_shift = self.octave_offset * 12
        progression = self.base_progressions[self.selected_progression]
        self.chords = [[note + octave_shift for note in chord] for chord in progression]
        
        # Actualitzar acord actual mantenint l'índex
        ...
        return {
            ...
            'octava': f"+{self.octave_offset}",
            'progr': f"{self.selected_progression+1}/{len(self.chord_progressions)}"
        }
